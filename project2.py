...
# Изменяем фон в зависимости от результата
if y_pred[0] == 1:
    st.markdown('<p style="background-color:red; color:white; padding:10px;">Клиент вероятно уйдет (отток).</p>', unsafe_allow_html=True)
else:
    st.markdown('<p style="background-color:green; color:white; padding:10px;">Клиент не уйдет (не будет оттока).</p>', unsafe_allow_html=True)
