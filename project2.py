...
sns.barplot(x=churn_data.index, y=churn_data.values, ax=ax, palette="Set2", edgecolor='black')
ax.set_ylabel('Доля клиентов', fontsize=12)
ax.set_xlabel('Отток (Churn)', fontsize=12)
ax.set_title('Доля оттока и не оттока', fontsize=14)
st.pyplot(fig)
...
fig, ax = plt.subplots(figsize=(8, 5))
sns.barplot(x=senior_data.index, y=senior_data.values, ax=ax, palette="Set3", edgecolor='black')
ax.set_ylabel('Доля клиентов', fontsize=12)
ax.set_xlabel('Пенсионер', fontsize=12)
ax.set_title('Доля пенсионеров и не пенсионеров', fontsize=14)
st.pyplot(fig)
...
fig, ax = plt.subplots(figsize=(8, 5))
sns.barplot(x=gender_data.index, y=gender_data.values, ax=ax, palette="Set2", edgecolor='black')
ax.set_ylabel('Доля клиентов', fontsize=12)
ax.set_xlabel('Пол клиента', fontsize=12)
ax.set_title('Доля женских и мужских половых клиентов', fontsize=14)
st.pyplot(fig)

# 5) График: оплата клиента за месяц и период
st.subheader('Оплата клиента за месяц и период')
fig, ax = plt.subplots(figsize=(8, 5))
sns.boxplot(x='SeniorCitizen', y='MonthlyCharges', data=data, ax=ax, palette="Set2")
ax.set_title('Оплата клиента за месяц и период', fontsize=14)
ax.set_xlabel('Пенсионер', fontsize=12)
ax.set_ylabel('Ежемесячная оплата', fontsize=12)
st.pyplot(fig)
